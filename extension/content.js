...
async function startAnalysisPipeline(shadowRoot) {
    ...
    if (props) {
        for (const [key, val] of Object.entries(props)) {
            ...
            // Security: Block script execution vectors
                ...
                continue;
            else if (key === 'href' && val.toLowerCase().trim().startsWith('javascript:'))
                continue;
...
function renderUI(shadowRoot, isPremium, result, isLoading) {
    ...
    // 상세 팩트체크 리스트
    const reasonsNodes = reasoning.map(r => h('div', { className: 'reason-item' }, h('span', {}, (r.type === 'penalty' || r.type === 'fact') ? '🚨' : '✅'), r.text));
    // 전체 UI 위젯 트리 생성
    ...
    // Header
    h('div', { className: 'header' }, h('div', { className: 'header-left' }, h('button', {
        className: 'close-btn',
        title: '판독기 숨기기',
        onClick: (e) => {
    ...
    // Score Container
    h('div', { className: 'score-container' }, h('div', { className: 'score-circle', style: '--score: ' + score + '%; --color: ' + color }, score + '%'), h('div', { className: 'conclusion' }, conclusion)), 
    // Details Section
    h('div', { className: 'details-section' }, h('div', { className: isPremium ? '' : 'premium-blur' }, h('div', { className: 'details-title' }, '상세 팩트체크 근거'), ...reasonsNodes), !isPremium ? h('div', { className: 'premium-overlay' }, h('div', { className: 'premium-lock-icon' }, '🔒'), h('button', {
