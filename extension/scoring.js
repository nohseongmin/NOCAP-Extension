 ...
 */
export function calculateCredibility(factScore, sourceScore, localSentimentScore, externalReasons = []) {
    // Revised weightings (Fact 70%, Source 30% - Visual removed)
    const W_FACT = 0.70;
    const W_SOURCE = 0.30;
    let baseScore = (factScore * W_FACT) + (sourceScore * W_SOURCE);
    let reasons = [...externalReasons];
    ...
    let conclusion = "";
    if (finalScore >= 80)
        conclusion = "신뢰도가 매우 높습니다. AI 분석 결과 논리적 일관성이 우수합니다.";
    else if (finalScore >= 50)
        conclusion = "정보가 섞여 있습니다. 일부 주장에 대해 비판적 수용이 필요합니다.";
    else if (finalScore >= 30)
        conclusion = "신뢰도가 낮습니다. 선동적이거나 비과학적인 주장이 포함되어 있습니다.";
    else
        conclusion = "허위 정보 또는 음모론일 가능성이 매우 높으므로 주의하십시오.";
    return {
        factScore,
        sourceScore,
